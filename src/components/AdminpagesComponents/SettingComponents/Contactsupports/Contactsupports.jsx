...
import AddContactsupports from './AddContactsupports';
...
function Contactsupports() {
  ...
  const filteredDocuments = documentList.filter((doc) =>
    doc.supports_data.toLowerCase().includes(filters.searchText.toLowerCase())
  );
  ...
  return (
    <section className="main-content-area">
      <Breadcrumb
        ...
        titleIcon="bx bx bx-support icon"
        buttons={[
          {
            label: (
          ...
          },
          {
            label: (
              <button className="addTaskBtn" onClick={() => setShowForm(true)}>
                <FaPlus className="me-1" /> Add Contact Supports
              </button>
            ),
          },
          {
            label: (
              <>
                Total Contact Supports : <span className="badge-length">{filteredDocuments.length}</span>
              </>
            ),
            className: 'btn-bg-2',
          },
      ...
      <div className={`slide-form-wrapper ${showForm ? 'show' : ''}`}>
        <AddContactsupports onClose={() => setShowForm(false)} />
      </div>
